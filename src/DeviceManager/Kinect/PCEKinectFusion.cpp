...
PCEKinectFusion::PCEKinectFusion()
	: m_threadOn(false)
	, m_threadPause(false)
	, m_pCache(NULL)
...
bool PCEKinectFusion::initializeKinectFusion(NUI_IMAGE_RESOLUTION depthResolution, NUI_IMAGE_RESOLUTION	colorResolution)
{
	HRESULT hr = S_OK;
	m_threadOn = false;
...
/// </summary>
/// <returns>S_OK on success, otherwise failure code</returns>
HRESULT PCEKinectFusion::ResetReconstruction()
{
...
/// </summary>
HRESULT PCEKinectFusion::CameraTrackingOnly()
{
...
HRESULT PCEKinectFusion::FusePointCloud(LONGLONG currentDepthFrameTime, bool bHasColor)
{
	HRESULT hr = S_OK;
	...
	/*hr = m_pVolume->DepthToDepthFloatFrame(pDepthBuffer, m_cDepthImagePixels * sizeof(NUI_DEPTH_IMAGE_PIXEL), m_pDepthFloatImage, m_fMinDepthThreshold, m_fMaxDepthThreshold, m_bMirrorDepthFrame);
	if (FAILED(hr))
	{
...
bool PCEKinectFusion::readMesh(PCEFusionMesh* pMesh)
{
	if(!m_pVolume || !pMesh)
		return false;
...
void PCEKinectFusion::runThread ()
{
	while (true)
	{
		if (!m_threadOn)
		...
		PCECompoundFrame* pFrame = m_pCache->getCompoundFrame(m_pCache->getFusionFrameToProcess());
		if (!pFrame)
			continue;
		...
		if(pAlignedColorBuffer && m_pAlignedColorImage)
		{
			INuiFrameTexture* pColorTexture = m_pAlignedColorImage->pFrameTexture;
			if(pColorTexture)
			{
				NUI_LOCKED_RECT LockedRect;
				HRESULT hr = pColorTexture->LockRect(0, &LockedRect, NULL, 0);
				if ( SUCCEEDED(hr) )
				{
					errno_t err = memcpy_s(PBYTE(LockedRect.pBits), pColorTexture->BufferLen(), pAlignedColorBuffer, pFrame->getColorBufferSize());
					bHasColor = (0 == err);
				}
				hr = pColorTexture->UnlockRect(0);
			}
		}
		USHORT* pDepthBuffer = pFrame->getDepthBuffer();
		if(pDepthBuffer && m_pDepthFloatImage)
		{
			/*INuiFrameTexture* pDepthFloatTexture = m_pDepthFloatImage->pFrameTexture;
			if(pDepthFloatTexture)
			{
				NUI_LOCKED_RECT LockedRect;
				HRESULT hr = pDepthFloatTexture->LockRect(0, &LockedRect, NULL, 0);
				float* pDepthFloatBuffer = reinterpret_cast<float*>(LockedRect.pBits);
				if(pDepthFloatBuffer)
				{
					UINT depthBufferSize = std::min(pFrame->getDepthWidth()*pFrame->getDepthHeight(), m_pDepthFloatImage->width*m_pDepthFloatImage->height);
					for (UINT i = 0; i < depthBufferSize; i++)
					{
						if(pDepthBuffer[i] == 0)
							pDepthFloatBuffer[i] = (float)(pDepthBuffer[i]) / PCEKinectCompoundFrame::cDepthToFloatCoefficient;
						else
							pDepthFloatBuffer[i] = 0.0f;
					}
				}
				hr = pDepthFloatTexture->UnlockRect(0);
			}*/

			NUI_DEPTH_IMAGE_PIXEL* pDepthImagePixelBuffer  = new(std::nothrow) NUI_DEPTH_IMAGE_PIXEL[m_cDepthImagePixels];
			UINT depthBufferSize = std::min(pFrame->getDepthWidth()*pFrame->getDepthHeight(), m_cDepthImagePixels);
			for (UINT i = 0; i < depthBufferSize; i++)
			{
				pDepthImagePixelBuffer[i].depth = pDepthBuffer[i];
				pDepthImagePixelBuffer[i].playerIndex = 0;
			}
			HRESULT hr = m_pVolume->DepthToDepthFloatFrame(pDepthImagePixelBuffer, m_cDepthImagePixels * sizeof(NUI_DEPTH_IMAGE_PIXEL), m_pDepthFloatImage, NUI_FUSION_DEFAULT_MINIMUM_DEPTH, NUI_FUSION_DEFAULT_MAXIMUM_DEPTH, m_bMirrorDepthFrame);
			if (FAILED(hr))
			{
				printf_s("Kinect Fusion NuiFusionDepthToDepthFloatFrame call failed.");
			}
		...
		if( FAILED(FusePointCloud(pFrame->getTimeStamp(), bHasColor)) )
		{
...
void PCEKinectFusion::startThread ()
{
	if(m_pVolume && !m_threadOn)
	{
		...
		m_Thread.reset (new boost::thread (boost::bind (&PCEKinectFusion::runThread, this)));
		m_threadOn = true;
...
void PCEKinectFusion::pauseThread()
{
	m_threadPause = true;
}

void PCEKinectFusion::stopThread ()
{
	m_threadOn = false;
	m_threadPause = false;
	if(m_Thread)
...
void PCEKinectFusion::updateBuffer(PCEDeviceCacheImp* pBuffer)
{
	if(pBuffer != m_pCache)
		m_cachedFrameIndex = 0;
	m_pCache = dynamic_cast<PCEDeviceCache*>(pBuffer);
}
