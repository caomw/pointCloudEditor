...
PCEPreviewTextureToTargetDX::PCEPreviewTextureToTargetDX(ID3D11Device* device)
: fDXDevice(device)
, fDXContext(NULL)
...
PCEPreviewTextureToTargetDX::~PCEPreviewTextureToTargetDX()
{
...
bool PCEPreviewTextureToTargetDX::renderTextureToTarget(MHWRender::MTexture* texture, MHWRender::MRenderTarget *target)
{
	if( fDXContext == NULL )
		return false;
...
void PCEPreviewTextureToTargetDX::initializeDrawTextureEffect()
{
	if( fDXContext == NULL )
		return;
	...
	if( fDrawTextureEffect == NULL )
	{
		...
		else
		{
			...
			MGlobal::displayWarning( "Failed to create effect <<^1s>>.", errorStr ) );
		}
	...
	if( fDrawTextureInputLayout == NULL && fDrawTexturePass != NULL )
	{
		...
		if( SUCCEEDED( hr ) )
		{
			...
			if( FAILED( hr ) )
			{
				MGlobal::displayWarning( "Failed to create input layout." ) );
			}
		}
	}
