...
#include "pointCloudTextureShaderOverride.h"
...
pointCloudTextureShaderOverride::pointCloudTextureShaderOverride(const MObject& obj)
...
, fObject(obj)
{
	...
	MString shaderLocation(
		...
		MString("/devkit/devkit/plug-ins/customTextureShader/CustomTextureBlock10.fx"));
	m_beautyShaderInstance = shaderMgr->getEffectsFileShader(shaderLocation, "Main");
}

MHWRender::MPxShaderOverride* pointCloudTextureShaderOverride::Creator(const MObject& obj)
{
	return new pointCloudTextureShaderOverride(obj);
}

pointCloudTextureShaderOverride::~pointCloudTextureShaderOverride()
{
...
MString pointCloudTextureShaderOverride::initialize(const MInitContext& initContext, MInitFeedback& initFeedback)
{
...
MHWRender::MShaderInstance* pointCloudTextureShaderOverride::shaderInstance() const
{
	if (m_inBeautyPass)
	{
...
void pointCloudTextureShaderOverride::updateDG(MObject object)
{
	MStatus status;
	MFnDependencyNode node(fObject, &status);
	if (status == MStatus::kSuccess)
	{
		pointCloudTextureShader* material = dynamic_cast<pointCloudTextureShader*>(node.userNode());
	}
}

void pointCloudTextureShaderOverride::updateDevice()
{
	// Set parameters:
...
void pointCloudTextureShaderOverride::endUpdate()
{
}

bool pointCloudTextureShaderOverride::handlesDraw(MHWRender::MDrawContext& context)
{
...
void pointCloudTextureShaderOverride::activateKey(MHWRender::MDrawContext& context, const MString& /*key*/)
{
...
bool pointCloudTextureShaderOverride::draw(MHWRender::MDrawContext& context, const MHWRender::MRenderItemList& renderItemList) const
{
	if (!m_shaderBound)
		return false;
...
void pointCloudTextureShaderOverride::terminateKey(MHWRender::MDrawContext& context, const MString& /*key*/)
{
	if (m_shaderBound)
	{
		if (m_inBeautyPass && m_beautyShaderInstance)
...
MHWRender::DrawAPI pointCloudTextureShaderOverride::supportedDrawAPIs() const
{
	// this example supports DX only
	return MHWRender::kDirectX11;
}

bool pointCloudTextureShaderOverride::isTransparent()
{
	return false;
}

bool pointCloudTextureShaderOverride::overridesDrawState()
{
	return false;
}

void pointCloudTextureShaderOverride::updateShaderInstance()
{
	MHWRender::MRenderer *renderer = MHWRender::MRenderer::theRenderer();
	if (!renderer)
		return;
...
void pointCloudTextureShaderOverride::buildAndUpdateCustomDataTextureViaMaya()
{
	MHWRender::MRenderer *renderer = MHWRender::MRenderer::theRenderer();
	if (!renderer)
		return;
...
void pointCloudTextureShaderOverride::generateCustomTexture()
{
	int textureSize = 256;
	...
	if (!m_customTextureData)
	{
		...
		for (int face=0 ; face<numSlice ; face++)
		{
			for (int j=0 ; j<textureSize ; j++)
			{
				...
				{
					m_customTextureData[index++] = (unsigned char)0;
